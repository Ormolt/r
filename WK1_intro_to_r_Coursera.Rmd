...
years_in_dataset <- range(present$year) 
print(years_in_dataset) 
```
...
ggplot(present, aes(x = year, y = prop_boys)) +
  ...
  labs(
    ...
    title = "Proportion of Boys Born Over Time"
  )
```
...
# Create a new variable 'more_boys'
present <- present %>%
  mutate(more_boys = boys > girls)

# Check the proportion of years with more boys
prop_more_boys <- mean(present$more_boys)

# Determine which statement is likely true
if (prop_more_boys == 1) {
  print("Every year there are more boys born than girls.")
} else if (prop_more_boys == 0) {
  print("Every year there are more girls born than boys.")
} else {
  print("Half of the years there are more boys born, and the other half more girls born.")
}
```
...
# Calculate the boy-to-girl ratio each year
present <- present %>% 
  mutate(prop_boy_girl = boys / girls)

# Plot the boy-to-girl ratio over time
ggplot(present, aes(x = year, y = prop_boy_girl)) +
  ...
  labs(
    x = "Year",
    y = "Boy-to-Girl Ratio",
    title = "Boy-to-Girl Ratio Over Time"
  ) 
```
