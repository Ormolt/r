...
cat("Number of flights to SFO in February:", num_flights, "\n") 
```
...
hist(sfo_feb_flights$arr_delay, 
     main = "Distribution of Arrival Delays for Flights to SFO in February", 
     xlab = "Arrival Delay (minutes)", 
     col = "lightblue")

# Summary statistics of arrival delays
...
# Calculate median and IQR for arr_delays by carrier
carrier_stats <- sfo_feb_flights %>%
  group_by(carrier) %>%
  summarize(median_delay = median(arr_delay, na.rm = TRUE),
            IQR_delay = IQR(arr_delay, na.rm = TRUE))

# Find the carrier with the highest IQR
carrier_with_highest_IQR <- carrier_stats %>%
  filter(IQR_delay == max(IQR_delay))

# Print the carrier with the highest IQR
print(paste0("Carrier with highest IQR: ", carrier_with_highest_IQR$carrier))
```
...
month_with_highest_delay <- monthly_avg_delays %>% 
  slice(1) %>% 
  pull(month)

# Print the result
cat("Month with the highest average departure delay:", month_with_highest_delay, "\n")
```
...
# Find the month with the highest median departure delay
month_with_highest_median_delay <- monthly_median_delays %>% 
  slice(1) %>% 
  pull(month)

# Print the result
cat("Month with the highest median departure delay:", month_with_highest_median_delay, "\n")
```
...
# Determine the airport with the best on-time departure rate
# Calculate on-time departure rate for each airport
airport_on_time_rates <- nycflights %>% 
  group_by(origin) %>% 
  summarize(ot_dep_rate = sum(dep_delay <= 0) / n()) %>% 
  arrange(desc(ot_dep_rate))

best_airport <- airport_on_time_rates %>% 
  slice(1) %>% 
  pull(origin)
# Print the result
cat("Airport with the best on-time departure rate:", best_airport, "\n") 
```
...
fastest_plane <- nycflights %>%
  mutate(avg_speed = (distance / air_time) * 60) %>%  # Calculate average speed in mph
  select(avg_speed, tailnum) %>%                   # Select relevant columns
  arrange(desc(avg_speed)) %>%                     # Sort by descending speed
  slice(1)                                         # Get the row with the highest speed
...
# Define on-time arrival based on arrival delay
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"))

# Calculate on-time arrival percentages for on-time and delayed departures
on_time_arrival_percentages <- nycflights %>%
  group_by(dep_delay > 0) %>% 
  summarize(on_time_arrival_prop = mean(arr_type == "on time"))

# Extract the proportion for delayed departures
prop_on_time_delayed_dep <- on_time_arrival_percentages %>% 
  slice(2) %>% 
  pull(on_time_arrival_prop)

# Print the result
cat("Proportion of delayed departing flights that arrive on time:", prop_on_time_delayed_dep, "\n") 
```